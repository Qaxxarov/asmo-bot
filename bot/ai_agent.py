...
MODELS = [GEMINI_MODEL, "gemini-1.5-flash", "gemini-2.0-flash"]
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as c:
                ...
                if r.status_code == 429:
                    logger.warning("Gemini 429 on %s, trying next...", model)
                    await asyncio.sleep(2)
                    continue
                if r.status_code != 200:
                    logger.error("Gemini %s status=%s body=%s", model, r.status_code, r.text[:200])
                    continue
                data = r.json()
                if "candidates" not in data or not data["candidates"]:
                    logger.error("Gemini no candidates: %s", str(data)[:200])
                    continue
                return data["candidates"][0]["content"]["parts"][0]["text"].strip()
        except Exception as e:
            logger.error("Gemini exception (%s): %s", model, e)
            continue
